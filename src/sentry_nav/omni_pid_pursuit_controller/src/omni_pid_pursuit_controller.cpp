...
#include "omni_pid_pursuit_controller/omni_pid_pursuit_controller.hpp"
...
namespace omni_pid_pursuit_controller
{
...
void OmniPidPursuitController::activate()
{
  ...
  // Initialize visualization time
  last_visualization_time_ = clock_->now();
  ...
  nav2_core::GoalChecker * /*goal_checker*/)
{
  ...
  // Transform path to robot base frame (before acquiring costmap lock)
  auto transformed_plan = transformGlobalPlan(pose);

  // Acquire costmap lock only when needed for collision checking
  ...
  double forward_dist, double backward_dist) const
{
  ...
  // Reduce visualization frequency for better performance (publish at 10Hz instead of control rate)
  auto current_time = clock_->now();
  if ((current_time - last_visualization_time_).seconds() > 0.1) {
...
PLUGINLIB_EXPORT_CLASS(
  omni_pid_pursuit_controller::OmniPidPursuitController, nav2_core::Controller)
