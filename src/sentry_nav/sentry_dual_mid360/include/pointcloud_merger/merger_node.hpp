...
private:
  ...
  void mergeCallback(
    const CustomMsg::ConstSharedPtr & primary_msg,
    const CustomMsg::ConstSharedPtr & secondary_msg);

  bool ensureSecondaryToCommonTransform();
  ...
  double min_dist_primary_m_;
  double min_dist_secondary_m_;
  int queue_size_;
  ...
  message_filters::Subscriber<CustomMsg> primary_sub_;
  message_filters::Subscriber<CustomMsg> secondary_sub_;
  std::unique_ptr<message_filters::Synchronizer<SyncPolicy>> sync_;

  std::unique_ptr<tf2_ros::Buffer> tf_buffer_;
  std::unique_ptr<tf2_ros::TransformListener> tf_listener_;
  bool has_secondary_to_common_transform_;
  Eigen::Isometry3d secondary_to_common_transform_;
  ...
  uint64_t drop_tf_;
  uint64_t drop_sync_;
  uint64_t drop_primary_min_dist_;
  uint64_t drop_secondary_min_dist_;
  uint64_t drop_primary_offset_overflow_;
  uint64_t drop_secondary_offset_overflow_;
