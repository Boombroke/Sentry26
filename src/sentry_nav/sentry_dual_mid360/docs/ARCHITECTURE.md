...
> **命名说明**：`primary_mid360` / `secondary_mid360` 只是历史标识符，**实际装法是绕 X 轴镜像的两颗雷达，不是一前一后 180° 反装**。调整物理安装角后名字不再携带方向语义，以相对外参（`T_secondary→primary`）为准。
...
graph LR
    ...
    D -->|/livox/lidar_primary<br/>CustomMsg| M[pointcloud_merger<br/>ApproximateTime 同步<br/>secondary→primary 刚体变换<br/>offset_time stable_sort<br/>header.stamp 单调保证]
    D -->|/livox/lidar_secondary<br/>CustomMsg| M
    D -->|/livox/imu<br/>front only| PL

    M -->|/livox/lidar<br/>CustomMsg<br/>frame=primary_mid360| PL[Point-LIO<br/>源码未改]
...
- **Merger 以上**：硬件 + Livox 驱动。dual 模式使用 `multi_topic=1 + xfer_format=1`，发布 `/livox/lidar_primary`、`/livox/lidar_secondary`、`/livox/imu`（front IMU 路由）。
- **Merger 内部**：`sentry_dual_mid360::MergerNode` 做四件事——同步、后向几何变换到前雷达系、按 `offset_time` `std::stable_sort`、强制 `header.stamp` 单调。
- **Merger 以下**：Point-LIO 只看到一路 `/livox/lidar`（CustomMsg，`frame_id=primary_mid360`），行为与单雷达完全一致；下游 odom_bridge / terrain_analysis / small_gicp / Nav2 对单/双雷达拓扑无感。
- **Back IMU**：Point-LIO 只消费 front IMU。Back IMU 仅作诊断（话题 `/livox/imu_secondary`），不进 LIO 状态估计，避免双 IMU 冲突。
...
- `livox_driver_dual_override.yaml`：
  ...
  - `multi_topic: 1` — 一台雷达一个话题，名字由 IP 决定（launch 侧把 `.144 → /livox/lidar_primary`、`.145 → /livox/lidar_secondary` 做 topic remap）。
...
3. 跑 `scripts/calib/verify_dual_mid360_sync.py` 采样 header.stamp 对齐情况。
...
1. **CAD 默认值**：`src/sentry_robot_description/resource/xmacro/wheeled_biped_real.sdf.xmacro` 定义
   - `primary_lidar_pose="0 0.102 0.3 -0.733 0.0 0.0"`（相对 `gimbal_pitch`：y=+10.2cm，roll=-42°）
   - `secondary_lidar_pose="0 -0.102 0.3 0.733 0.0 0.0"`（相对 `gimbal_pitch`：y=-10.2cm，roll=+42°，X 轴镜像）
...
2. **标定工具**：Multi_LiCa 离线标定（依赖 teaserpp_python / open3d / pandas / ros2_numpy / scipy）。入口脚本：`scripts/calib/calibrate_dual_mid360.sh`。命令形如
   ...
   scripts/calib/calibrate_dual_mid360.sh --bag <calib_bag_dir> --output-report <path> --bootstrap --write-xmacro
   # 日常微调 / xmacro 已是上次标定过的好值（strict 模式）：
   scripts/calib/calibrate_dual_mid360.sh --bag <calib_bag_dir> --output-report <path> --write-xmacro
   ```
...
4. **同时产出相对与绝对测量**：精度检查文件（`task-11-precision-check.txt`）一级字段同时包含：
   - `relative_secondary_in_primary_xyz_rpy_rad/deg`：`T_secondary→primary` 纯点云配准结果，**与 xmacro 值无关**。
     以后 `primary_lidar_pose` 定版 / 修订时，用这个相对值一次矩阵乘即可推出新
     `secondary_lidar_pose`，不需要重录 bag / 重标。
   - `candidate_secondary_lidar_pose_xyz_rpy_rad`：`primary_base @ T_secondary→primary` 的绝对 pose，
     绑定当前 xmacro 的 `primary_lidar_pose`；front 一旦修订，这个值即失效，必须重算或重标。
...
| 方向 | Topic | 类型 | 备注 |
|------|-------|------|------|
| Sub | `/livox/lidar_primary` | `livox_ros_driver2/CustomMsg` | front Mid360 raw |
| Sub | `/livox/lidar_secondary`  | `livox_ros_driver2/CustomMsg` | back Mid360 raw |
| Pub | `/livox/lidar`       | `livox_ros_driver2/CustomMsg` | 融合结果，`frame_id=primary_mid360` |
...
提供 `scripts/qa/qa_merger_latency.py`：

- 默认 mock 模式不依赖硬件，生成 CustomMsg 形状点云，计时三个热点（min-distance 过滤 / secondary→primary 刚体变换 / stable_sort）。
...
`scripts/codegen/generate_pointlio_overrides.py` 在 `colcon build --packages-select sentry_dual_mid360` 期间被 CMake `add_custom_command` 触发，只读两个 xmacro 输入：

- **Layer A**：`wheeled_biped_real.sdf.xmacro` 的 `primary_lidar_pose` → 决定 `mapping.gravity` 与 `gravity_init`（CAD 下俯角 30°）。
...
- `scripts/codegen/verify_pointlio_overrides_fresh.py` 是 CI/台架门控：对当前 xmacro 输入重算一次，与安装 YAML 做字段级 diff，不一致就拒绝放行。
...
graph TB
    ...
    primary_mid360 --> front_mid360_imu
...
- `secondary_mid360` 相对 `gimbal_pitch` 以 X 轴镜像姿态安装（roll 取 `primary_mid360` 的相反数、无 yaw offset），TF fragment 由本包 `urdf/secondary_mid360.sdf.xmacro` 提供（跨包 `xmacro_include` 由 `sentry_robot_description` 调用，见根 `AGENTS.md` §9）。**不是 yaw=π 的前后反装**。
- `*_mid360_imu` 是 IMU 出厂 anchor（Layer B）；`front_mid360_imu` 的变换是 Point-LIO `extrinsic_T/R` 的派生源，`back_mid360_imu` 仅诊断用，不进 LIO。
- `common_frame` / merger `output` frame 固定为 `primary_mid360`；Point-LIO 以该 frame 读取点云并在内部维护 `camera_init`。
...
- 命令：
  ...
  ros2 topic hz /livox/lidar_primary /livox/lidar_secondary
  ```
...
- 命令：
  ...
  ros2 topic hz /red_standard_robot1/livox/lidar_primary_points
  ros2 topic hz /red_standard_robot1/livox/lidar_secondary_points
  ros2 topic info /red_standard_robot1/livox/lidar_primary
  ros2 topic info /red_standard_robot1/livox/lidar_secondary
...
- ~~**T11（外参标定 live 分支）**~~：**已清除（2026-05-11）**。依赖齐全，`--bootstrap` 首次标定链路跑通并成功回写 xmacro，相对外参 `T_secondary→primary` 固化于 `logs/evidence/task-11-relative-extrinsic.md`。下一次标定等 `primary_lidar_pose` 机械定版后再跑 strict 模式。
- **T15（仿真双雷达）**：`rmu_gazebo_simulator` 的 gz→ROS 桥已切换成 dual 布局（前后 `gpu_lidar` 到 `livox/lidar_primary_points` / `livox/lidar_secondary_points` PointCloud2，前 IMU 到 `livox/imu`，后 IMU 到 `livox/imu_secondary`）。仿真 dual mode 通过本包 `SimPointCloudToCustomMsgNode` 两实例把 PC2 转成 Livox `CustomMsg` 喂 `MergerNode`，Point-LIO 消费合并后的 `livox/lidar` CustomMsg。实际 Gazebo smoke 仍需在能启动仿真的机器上跑。
...
- **Front / Back 视野配比调优**：当前两颗都下俯 30°；如果实测后向掠地需求更高，可改 `secondary_lidar_pose` 的 pitch，走 xmacro 单源 + codegen，一次 build 同步 `extrinsic_T/R` 与下游参数。
...
- 本包 `scripts/calib/calibrate_dual_mid360.sh`：Multi_LiCa 外参标定 + 安全 xmacro 写回入口。
