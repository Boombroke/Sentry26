...
- **关键参数配置**:
    ...
    | gravity | [0, 0, -9.81] (仿真) | Point-LIO 重力状态初值；机体系姿态由 IMU 初始化自动对齐 |

- 仿真 `wheeled_biped_sim.sdf.xmacro` 的 Mid360 相对 `gimbal_pitch` 额外固定下俯约 30°，用于覆盖近场低矮底座；当前实车 `wheeled_biped_real.sdf.xmacro` 使用实测 15° 下俯外参 `-0.05 0 0.05 0.0 0.2617993877991494 3.141592653589793`。如果后续出现侧装或倒装，也应继续只写在 `gimbal_pitch → front_mid360` 的传感器外参中。这个安装角留在机器人描述 / TF 中，不直接写入 Point-LIO 的 `gravity` 参数。
