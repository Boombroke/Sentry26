...
[Secondary Mid360 192.168.1.145] --+--> [/livox/lidar_secondary  (CustomMsg)] --+     secondary→primary 刚体变换
                               ...
                               +--> [/livox/imu_secondary] (仅诊断，不进 LIO)      header.stamp 单调]
                                                                             |
                                                                             v
                                                             [/livox/lidar (CustomMsg, frame=primary_mid360)]
                                                                             |
+-----------------------+      +-----------------------+      +-----------+-+---------+
...
- **硬件布局**：两颗 Livox Mid360 都固定到 `gimbal_pitch`，相对 `gimbal_pitch` 绕 X 轴镜像（roll 反向、无 yaw offset），水平 FOV 合并近 360°。`front/back` 只是历史 frame 标识符，不代表物理前后方位。
- **IMU 路由**：只有前 Mid360 的 IMU 进 Point-LIO（话题 `/livox/imu`）；后 Mid360 的 IMU 仅作诊断（`/livox/imu_secondary`），不进 LIO 状态估计，避免双 IMU 冲突。
- **驱动模式**：Livox driver 运行在 `multi_topic=1 / xfer_format=1` 下，发布两路独立 CustomMsg；launch 层用 per-device IP 后缀 remap 出 `/livox/lidar_primary` 与 `/livox/lidar_secondary`。
- **Merger 节点**：`sentry_dual_mid360::MergerNode` 用 `message_filters::ApproximateTime` + `setMaxIntervalDuration` 双重门控同步，back 点云在原生系做 min-distance 过滤后，经 `secondary_mid360 → primary_mid360` 刚体变换合并到前雷达系；逐点 `offset_time` 经 `std::stable_sort` 保持单调，输出 `/livox/lidar` 的 `header.stamp` 强制单调递增、`timebase=0`、保留 `tag / reflectivity / line` 字段。
- **Point-LIO 零改动**：Point-LIO 订阅 `/livox/lidar` 当单雷达消费，源码不分叉。`extrinsic_T / extrinsic_R / mapping.gravity / gravity_init` 由 `sentry_dual_mid360/scripts/codegen/generate_pointlio_overrides.py` 在 `colcon build` 时从 xmacro 单源派生到 `install/sentry_dual_mid360/share/sentry_dual_mid360/config/pointlio_dual_overrides.yaml`，launch 以 `IfCondition(use_dual_mid360)` 叠加该 override。
...
- **输入话题（双雷达实车链路，`use_dual_mid360:=True` 默认）**:
    - `livox/lidar` (`livox_ros_driver2/msg/CustomMsg`): `pointcloud_merger` 输出的融合 CustomMsg，`frame_id=primary_mid360`，逐点保留 `offset_time / reflectivity / tag / line`。
    - `livox/imu` (sensor_msgs/msg/Imu): 前 Mid360 IMU（唯一进 LIO 的 IMU；后 Mid360 IMU 走 `/livox/imu_secondary` 仅诊断）。
...
- **输入话题（仿真，`use_dual_mid360:=True`，默认）**:
    - Gazebo gz→ROS 桥发布 `livox/lidar_primary_points` + `livox/lidar_secondary_points`（`sensor_msgs/msg/PointCloud2`）以及 `livox/imu` + `livox/imu_secondary`（`sensor_msgs/msg/Imu`）。
    - `sentry_dual_mid360::SimPointCloudToCustomMsgNode` 前后两实例把两路 PC2 转换成 `livox_ros_driver2/msg/CustomMsg`（`livox/lidar_primary` / `livox/lidar_secondary`），再由 `MergerNode` 合并成 `livox/lidar` CustomMsg 喂 Point-LIO；与实车共用同一条 CustomMsg + merger 链路，Point-LIO 源码零改动。
- **输入话题（仿真，`use_dual_mid360:=False`）**:
    - `ign_sim_pointcloud_tool` 订阅 `livox/lidar_primary_points`（base nav2_params.yaml 指定），转换成 `velodyne_points`（`sensor_msgs/msg/PointCloud2`）供 Point-LIO 单雷达回退链路使用。
- **输出话题**:
    - `aft_mapped_to_init` (nav_msgs/msg/Odometry): 优化后的里程计位姿。
    - `cloud_registered` (sensor_msgs/msg/PointCloud2): 投影到世界坐标系下的实时点云。
- **坐标变换 (TF)**: 发布 `lidar_odom -> primary_mid360`（`primary_mid360` 是融合输出的 common frame，Point-LIO 只看到一条主雷达链路）。
...
- **实车双 Mid360 外参（当前值）**：两颗 Mid360 挂 `gimbal_pitch`，绕 X 轴镜像安装（`primary_lidar_pose="0 0.102 0.3 -0.733 0.0 0.0"`、`secondary_lidar_pose="0 -0.102 0.3 0.733 0.0 0.0"`），两组 pose 只差 y 分量和 roll 符号，**不带 yaw=π**。机械调整后 `secondary_lidar_pose` 必须用 `scripts/calib/calibrate_dual_mid360.sh --bootstrap --write-xmacro` 重标。安装角只留在机器人描述 / TF，**不**直接写入 Point-LIO `gravity`；dual 模式下 `gravity / gravity_init / extrinsic_T / extrinsic_R` 都由 codegen 从 xmacro 派生。仿真 `wheeled_biped_sim.sdf.xmacro` 的 Mid360 仍保留 30° 下俯用于覆盖近场低矮底座（仿真布局未跟进实车镜像调整，不影响仿真 SLAM）。
- **Point-LIO 零改动原则**: 双 Mid360 融合发生在 Point-LIO 之前；不对 `src/third_party/point_lio/` 做任何源码分叉。参数层面的切换通过 launch `IfCondition(use_dual_mid360)` 把 `install/sentry_dual_mid360/share/sentry_dual_mid360/config/pointlio_dual_overrides.yaml` 作为后置层叠加到单雷达默认 YAML 上；该 override YAML 是 build 产物，不在源码 `config/` 提交、不可手编辑（见 [参数调优 §1.5 / §七](TUNING_GUIDE.md) 与 `scripts/codegen/verify_pointlio_overrides_fresh.py`）。
         ...
         └─ chassis (底盘刚体原点，yaw ≡ base_footprint)
                 └─ gimbal_yaw (当前实车 profile 为静态 TF；仿真可动态控制)
                     └─ gimbal_pitch (当前实车 profile 为静态 TF；仿真可动态控制)
                         ├─ primary_mid360 (前 Livox Mid360，随云台旋转)
                         │   └─ front_mid360_imu (Layer B IMU 出厂 anchor，进 Point-LIO)
                         └─ secondary_mid360  (第二颗 Livox Mid360，X 轴镜像安装，随云台旋转)
...
- **两颗 Mid360 均挂在 `gimbal_pitch` 上**，随云台同步旋转；两颗绕 X 轴镜像安装（roll 反向、无 yaw offset），通过俯仰角差错开扫描面实现近 360° 水平覆盖。`odom_bridge` 通过每帧 `lookupTransform(lidar_frame → base_frame)` 消化云台旋转对底盘位姿估计的影响，使 Nav2 侧 `base_footprint` 位姿与云台解耦。
...
- 双 Mid360 外参只能改 xmacro（唯一真相源）：`wheeled_biped_real.sdf.xmacro` 的 `primary_lidar_pose / secondary_lidar_pose` 和 `sentry_dual_mid360/urdf/mid360_imu_tf.sdf.xmacro` 的 IMU factory pose；改完重跑 `colcon build --packages-select sentry_dual_mid360`，codegen 会同步刷新 `pointlio_dual_overrides.yaml`。
- `secondary_lidar_pose` 可以通过点云配准自动标定：`bash src/sentry_nav/sentry_dual_mid360/scripts/calib/calibrate_dual_mid360.sh --bag <dir> --bootstrap --write-xmacro` 会把 Multi_LiCa 测出的 secondary→primary 相对外参与 `primary_lidar_pose` 合成后回写 xmacro。新手看 `src/sentry_nav/sentry_dual_mid360/docs/CALIBRATION_QUICKSTART.md`。`primary_lidar_pose` 仍属机械测量，不在标定管线内。
