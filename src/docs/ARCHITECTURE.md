...
- 仿真 wheeled_biped 的 Mid360 相对 `gimbal_pitch` 额外固定下俯约 30°，用于覆盖近场低矮底座；如果后续出现侧装或倒装，也应继续只写在 `gimbal_pitch → front_mid360` 的传感器外参中。这个安装角留在 Gazebo 模型 / TF 中，不直接写入 Point-LIO 的 `gravity` 参数。
